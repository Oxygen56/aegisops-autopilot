...
| Uses Qwen Cloud APIs / required tools | `src/server/agent/qwenClient.ts` uses the Qwen/DashScope OpenAI-compatible endpoint and accepts `QWEN_API_KEY` or `DASHSCOPE_API_KEY`; `reports/live_qwen_smoke_proof.md` verifies a one-shot `qwen-plus` call with five tool schemas | Verified |
