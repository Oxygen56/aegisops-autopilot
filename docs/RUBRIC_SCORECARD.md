...
| --- | ---: | --- | --- |
| Innovation & AI Creativity | 30% | Qwen Cloud reasoning path, OpenAI-compatible live function tool loop, five custom incident tools, OpenAPI spec, MCP stdio server, persistent memory, agent council, and human approval gate | `docs/QWEN_TOOLS.md`, `reports/qwen_integration_audit.md`, `agents/aegisops/openapi.yaml`, `agents/aegisops/cap-manifest.json`, `src/server/mcp/aegisopsMcp.ts`, `reports/judge_demo_transcript.md` |
...
| Presentation & Documentation | 15% | Judge quickstart, architecture diagram, visible dashboard rubric-evidence panel, demo script, video pack, video asset audit, Devpost copy, submission audit, reproducible run ledger | `docs/JUDGE_QUICKSTART.md`, `docs/ARCHITECTURE.md`, `src/client/main.tsx`, `docs/DEMO_SCRIPT.md`, `docs/VIDEO_SUBMISSION.md`, `reports/video_asset_audit.md`, `submissions/devpost_fields.md`, `reports/submission_audit.md`, `reports/experiment_board.md` |
...
| Invokes external tools | Uses `log_search`, `metric_probe`, `change_graph`, `policy_check`, and `remediation_simulator`, available to Qwen through a live Function Calling loop and exposed through HTTP/OpenAPI and MCP |
