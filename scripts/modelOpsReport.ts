...
const rows = listIncidents().map((incident) => {
  ...
  return {
    ...
    offlineQwenRequests: 1,
    maxLiveQwenRequests: 3,
...
const lines = [
  ...
  ...rows.map(
    (row) =>
      `| ${row.incidentId} | ${row.severity} | ${row.offlineQwenRequests} | ${row.maxLiveQwenRequests} | ${row.qwenFunctionTools} | ${row.evidenceTools} | ${row.localToolBudgetMsApprovedPath}ms | ${row.estimatedInputTokens} | ${row.estimatedOutputTokens} | ${row.estimatedTotalTokens} | ${row.humanApprovalRequired ? "required" : "policy-auto"} |`
  ),
  "",
  "## Suite Summary",
  "",
