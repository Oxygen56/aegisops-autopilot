...
| VERIFIED | Project uses Qwen Cloud models and API surface | `src/server/agent/qwenClient.ts`<br>`reports/qwen_integration_audit.md`<br>`docs/QWEN_TOOLS.md` | Automated audit verifies endpoint, credentials, offline fallback, tool schemas, OpenAPI tools, MCP, and CI coverage. |
...
| VERIFIED | Custom tool system is portable through Qwen Function Calling, HTTP/OpenAPI, and MCP | `docs/QWEN_TOOLS.md`<br>`agents/aegisops/openapi.yaml`<br>`src/server/mcp/aegisopsMcp.ts`<br>`tests/orchestrator.test.ts` | One registry backs Qwen request schemas, HTTP endpoints, and MCP stdio calls. |
