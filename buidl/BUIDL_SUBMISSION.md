...
9. Lets Qwen select incident tools through an OpenAI-compatible Function Calling loop and exposes the same registry through OpenAPI plus MCP stdio for portable agent integration.
...
- Qwen Cloud integration: `src/server/agent/qwenClient.ts`, including OpenAI-compatible `tools`, live `tool_calls`, and `role=tool` result messages.
...
- Qwen integration audit: `reports/qwen_integration_audit.md` verifies the Qwen endpoint, credential env vars, offline fallback, Function Calling loop, OpenAPI tools, MCP stdio, and CI coverage.
...
- Innovation: combines Qwen Cloud reasoning, OpenAI-compatible function tool calls, persistent memory, multi-agent review, policy-aware tools, MCP/OpenAPI custom tool surfaces, and approval-gated remediation.
